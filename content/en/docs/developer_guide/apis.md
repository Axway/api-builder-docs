...
| parameters | false | Input parameters required to execute the API endpoint. This is an object of key-value pairs, where the key is the name of the parameter, and the value is an object with the following properties:<br /><br /><ul><li> `optional` (Boolean): Determines if the parameter is optional (`true`) or required (`false`).<br /><li> `type` (String): the type of input parameter: `path`, `query`, or `body`.<br /><li> `description` (String): used for generating API documentation. |</ul>
